...
import org.springframework.context.annotation.Bean; // Define que um metodo cria um bean gerenciado pelo Spring.
...
public class SecurityConfig {
    ...
    @Bean // @Bean: Define que este metodo cria um **bean** gerenciado pelo Spring. Um **bean** é um objeto criado, configurado e gerenciado pelo Spring, controlando seu ciclo de vida e disponibilizando-o para ser injetado em outras partes da aplicação. Métodos anotados com @Bean geralmente retornam instâncias de objetos que o Spring deve gerenciar.
    public AuthenticationManager authenticationManager(AuthenticationConfiguration config) throws Exception { // O AuthenticationManager gerencia o processo de autenticação. Ele é configurado com base na configuração definida em AuthenticationConfiguration.
        return config.getAuthenticationManager();
    }

    @Bean // Cria um bean para o PasswordEncoder. O PasswordEncoder é usado para criptografar e verificar senhas.
    public PasswordEncoder passwordEncoder() { // Retorna uma instância de BCryptPasswordEncoder, que usa o algoritmo BCrypt para criptografar senhas. **BCrypt** é um dos algoritmos mais seguros para hash de senhas, pois é resistente a ataques de força bruta e tem um fator de complexidade ajustável. O Spring Security usa isso para armazenar senhas de forma segura no banco de dados.
    ...
    public AuthenticationProvider authenticationProvider() { // DaoAuthenticationProvider: Provê autenticação baseada em dados persistidos no banco (usando o padrão DAO).
    ...
    @Bean // Metodo que configura a cadeia de filtros de segurança. O SecurityFilterChain define a sequência de filtros de segurança a serem aplicados em todas as requisições HTTP. Cada requisição HTTP passa por uma cadeia de filtros que determinam se a requisição deve ser autenticada ou autorizada.
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception { // Configuração da segurança HTTP usando o HttpSecurity. O HttpSecurity permite personalizar a configuração de segurança baseada na web. Desabilita a proteção CSRF (Cross-Site Request Forgery).
        http
                .csrf(csrf -> csrf.disable()) // **CSRF (Cross-Site Request Forgery)**: O CSRF é uma vulnerabilidade que permite que um atacante faça requisições maliciosas em nome de um usuário autenticado sem o seu consentimento. No entanto, o CSRF geralmente não é necessário quando usamos tokens JWT, porque a proteção CSRF é projetada principalmente para sessões baseadas em cookies. Como a autenticação com JWT usa um token que é enviado explicitamente pelo cliente em cada requisição (geralmente no cabeçalho Authorization), o CSRF pode ser desabilitado. Define as regras de autorização para as requisições HTTP.
                ...
                .oauth2Login(oauth2 -> oauth2.loginPage("/auth/login")) // Configura o login via OAuth2. **oauth2Login()** define o fluxo de login OAuth2, permitindo que usuários se autentiquem usando provedores OAuth2 como GitHub ou Google.
                .oauth2ResourceServer(oauth2 -> oauth2.jwt(jwt -> jwt // Configura o OAuth2 Resource Server para verificar tokens JWT. **oauth2ResourceServer()** habilita o servidor de recursos OAuth2 para validar tokens JWT recebidos no cabeçalho Authorization.
                        .jwtAuthenticationConverter(jwtAuthenticationConverter()) // Usa o JwtAuthenticationConverter para converter tokens JWT em autenticações Spring Security.
                ))
    ...
    @Bean // Metodo que configura o JwtAuthenticationConverter. O JwtAuthenticationConverter é responsável por converter o token JWT em uma autenticação Spring Security.
