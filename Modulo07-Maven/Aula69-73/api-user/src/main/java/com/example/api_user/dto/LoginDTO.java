...
@NoArgsConstructor // Anotação @NoArgsConstructor: Gera automaticamente um **construtor vazio** (sem parâmetros) para a classe. Isso é útil em muitos cenários, como ao trabalhar com frameworks como o Spring ou JPA, que exigem um construtor sem parâmetros para criar instâncias de objetos.
@Data //  @Data: A anotação @Data do Lombok combina várias funcionalidades em uma única anotação. Ela gera automaticamente os seguintes métodos: **Getters e Setters** para todos os campos da classe (`username` e `password`). **toString()**: Um metodo para retornar uma representação de string do objeto. **equals() e hashCode()**: Métodos para comparar objetos com base em seus atributos. **RequiredArgsConstructor**: Construtor que inclui os campos obrigatórios (embora já seja substituído pelo @AllArgsConstructor e @NoArgsConstructor).
public class LoginDTO { // Campos que serão utilizados para login
